...
class EKF(object):
    ...
    def use_wall_angle(self,angle):
        # compute angle relative to closest X/Y axis
        relang = fmod(self.state[2,0]+12*pi,pi/2)
        if relang > pi/4:
            relang -= pi/2
        # presumed wall angle (multiple of pi/2)
        wallang = self.state[2,0] - relang
        robot_from_wall = pi/2-angle
        rospy.loginfo('Using wall, current angle %5.3f, %5.3f to axis, presumed wall angle %5.3f, robot %5.3f from wall',self.state[2,0],relang,wallang,robot_from_wall)
        if abs(relang-robot_from_wall) < pi/12:
            newang = wallang + robot_from_wall
            self.state[2,0] = newang
            rospy.loginfo('Pose would have been updated by wall, new angle %5.3f state %5.3f',newang,self.state[2,0])
