...
class CorobotMonitorUI(Tk):
    ...
    def setRawnavMsg(self, txt):
        txt = txt.split()
        if len(txt) == 3:
            self.lazcanvas.delete("forceOld")
            nav = [float(txt[0][1:-1]),float(txt[1][:-1])]
            self.lazcanvas.create_line(200, 600, math.sin(nav[1]) * nav[0] * -100 + 200,600 - math.cos(nav[1])* nav[0] * 100, arrow = LAST, width=3, fill="blue", tags = "forceOld" )
            nav = [1, float(txt[1][:-1])]
            self.lazcanvas.create_line(200, 600, math.sin(nav[1]) * nav[0] * -100 + 200,600 - math.cos(nav[1])* nav[0] * 100, fill="blue", tags = "forceOld" )
            return
        self.lazcanvas.delete("force")
        self.Nav = [float(txt[0][1:-1]),float(txt[1][:-1])]
        self.lazcanvas.create_line(200, 600, math.sin(self.Nav[1]) * self.Nav[0] * -100 + 200,600 - math.cos(self.Nav[1])* self.Nav[0] * 100, arrow = LAST, width=3, tags = "force" )
        text = "RawNav: <{0:4.2f}".format(float(txt[0][1:-1]))
        text += ", {0:4.2f}>".format(float(txt[1][:-1]))
        self.rawnavinfo.configure(text=text)

    def setPerpForce(self, txt):
        txt = txt.split()
        if len(txt) == 1:
            self.lazcanvas.delete("perpForce")
            return
        force = [1,float(txt[1][:-1])]
        self.lazcanvas.create_line(200, 600, math.sin(force[1]) * force[0] * -100 + 200,600 - math.cos(force[1])* force[0] * 100, tags = "perpForce", fill="green")
        force[0] = float(txt[0][1:-1])
        self.lazcanvas.create_line(200, 600, math.sin(force[1]) * force[0] * -100 + 200,600 - math.cos(force[1])* force[0] * 100, arrow = LAST, tags = "perpForce", fill="green", width=3)
        
    def setRepForce(self, txt):
        txt = txt.split()
        if len(txt) == 1:
            self.lazcanvas.delete("repForce")
            return
        force = [1,float(txt[1][:-1])]
        self.lazcanvas.create_line(200, 600, math.sin(force[1]) * force[0] * -100 + 200, 600 - math.cos(force[1])* force[0] * 100, tags = "repForce", fill="red")
        force[0] = float(txt[0][1:-1])
        self.lazcanvas.create_line(200, 600, math.sin(force[1]) * force[0] * -100 + 200, 600 - math.cos(force[1])* force[0] * 100, arrow = LAST, tags = "repForce", fill="red" , width=3)
	...
	self.goal = self.canvas.create_oval((pos[0]/.1312) - 73, self.map.height() - (pos[1]/.1312) +5, (pos[0]/.1312) - 63, self.map.height() - (pos[1]/.1312) - 5, fill ='green')
    ...
    def setLaserData(self, laserData):
        self.lastLaser = laserData

    def drawLaserMap(self):
        Omin = self.lastLaser.angle_min
        rmin = self.lastLaser.range_min
        rmax = self.lastLaser.range_max
        Oinc = self.lastLaser.angle_increment
        dist = self.lastLaser.ranges
    ...
    def drawWalls(self):
        wallDat = self.wallDat
        if wallDat.is_wall_left:
            r = wallDat.rleft
            th = wallDat.thetaleft*math.pi/180
            if wallDat.thetaleft >= 45.00 and wallDat.thetaleft <=135.00:
                x1 = 50.00
                y1 = (r - (x1*math.cos(th)))/math.sin(th)
                x2 = 550.0
                y2 = (r - (x2*math.cos(th)))/math.sin(th)
                self.lazcanvas.create_line(int(200-y1),int(600-x1),int(200-y2),int(600-x2), fill="red", width=5.0, tag = 'kinectVision')
            else:
                y1 = 175.00
                x1 = (r - (y1*math.sin(th)))/math.cos(th)
                y2 = -175.0
                x2 = (r - (y2*math.sin(th)))/math.cos(th)
                self.lazcanvas.create_line(int(200-y1),int(600-x1),int(200-y2),int(600-x2), fill="red", width=5.0, tag = 'kinectVision')

            # y1 = 50.00
            # x1 = ((r-50.00) - (y1*math.sin(th)))/math.cos(th)
            # y2 = 150.0
            # x2 = ((r-50.00) - (y2*math.sin(th)))/math.cos(th)
            # self.lazcanvas.create_line(int(200-y1),int(600-x1),int(200-y2),int(600-x2), fill="black", width=5.0, tag = 'kinectVision')


            x1 = 200.00
            y1 = 600.00
            x2 = (200.00 - (r*math.sin(th)))
            y2 = (600.00 - (r*math.cos(th)))
            self.lazcanvas.create_line(x1,y1,x2,y2, fill="red", width=5.0, tag = 'kinectVision')

        if wallDat.is_wall_right:
            r = wallDat.rright
            th = wallDat.thetaright*math.pi/180
            if wallDat.thetaright >= 45.00 and wallDat.thetaright <=135.00:
                x1 = 50.00
                y1 = (r - (x1*math.cos(th)))/math.sin(th)
                x2 = 550.0
                y2 = (r - (x2*math.cos(th)))/math.sin(th)
                self.lazcanvas.create_line(int(200-y1),int(600-x1),int(200-y2),int(600-x2), fill="blue", width=5.0, tag = 'kinectVision')
            else:
                y1 = 175.00
                x1 = (r - (y1*math.sin(th)))/math.cos(th)
                y2 = -175.0
            ...
            x2 = (200.00 - (r*math.sin(th)))
            y2 = (600.00 - (r*math.cos(th)))
            self.lazcanvas.create_line(x1,y1,x2,y2, fill="blue", width=5.0, tag = 'kinectVision')
